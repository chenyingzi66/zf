...
export default function Topbar({
  ...
  /** 旧界面退出前有二次确认;模版原来只弹 toast 不清 token,这里补上清凭据 */
  const handleLogout = async () => {
    const confirmed = await confirm({ title: '提示', description: '确定退出登录?', confirmText: '退出', tone: 'default' });
    if (!confirmed) return;
    clearAuth();
    toast.success('已退出登录');
          ...
          <div style={{ position: 'relative', marginLeft: 4 }}>
            ...
            {accountMenuOpen && (
              ...
              >
                ...
                >
                  <KeyRoundIcon size={16} color="var(--primary)" />
                  修改密码
                </button>
                <div style={{ height: 1, margin: '4px 2px', background: 'var(--border)' }} />
                <button
